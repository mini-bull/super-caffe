...
#include "caffe/layers/cropnew_layer.hpp"
...
void CropNewLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const CropNewParameter& param = this->layer_param_.cropnew_param();
...
void CropNewLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < input_dim; ++i) {
    ...
    if (i >= start_axis) {
      ...
      CHECK_GE(bottom[0]->shape(i) - crop_offset, 512)
          << "the crop for dimension " << i << " is out-of-bounds with "
          << "size 512" << " and offset " << crop_offset;
    }
...
template <typename Dtype>
void CropNewLayer<Dtype>::crop_copy(const vector<Blob<Dtype>*>& bottom,
             const vector<Blob<Dtype>*>& top,
