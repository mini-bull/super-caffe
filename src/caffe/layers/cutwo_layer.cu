...
template <typename Dtype>
void CutwoLayer<Dtype>::cutwo_copy_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (cur_dim + 2 < top[0]->num_axes()) {
    ...
    for (int i = 0; i < top[0]->shape(cur_dim); ++i) {
      indices[cur_dim] = i;
      cutwo_copy_gpu(bottom, top, offsets, indices, cur_dim+1,
...
template <typename Dtype>
void CutwoLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  std::vector<int> indices(top[0]->num_axes(), 0);
  const Dtype* bottom_data = bottom[0]->gpu_data();
  Dtype* top_data = top[0]->mutable_gpu_data();
  cutwo_copy_gpu(bottom, top, offsets, indices, 0, bottom_data, top_data, true);
}

template <typename Dtype>
void CutwoLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  NOT_IMPLEMENTED;
}


INSTANTIATE_LAYER_GPU_FUNCS(CutwoLayer);
