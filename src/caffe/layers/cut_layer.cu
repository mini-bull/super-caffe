...
template <typename Dtype>
void CutLayer<Dtype>::cut_copy_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (cur_dim + 2 < top[0]->num_axes()) {
    ...
    for (int i = 0; i < top[0]->shape(cur_dim); ++i) {
      indices[cur_dim] = i;
      cut_copy_gpu(bottom, top, offsets, indices, cur_dim+1,
...
void CutLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  cut_copy_gpu(bottom, top, offsets, indices, 0, bottom_data, top_data, true);
  cut_copy_gpu(bottom, top, offsets_cut, indices, 0, bottom_data, top_data_1, true);
}
