    ...
    for (int j = 0; j < top_k_; ++j) {
      if (out_max_val_) {
        if (has_axis_) {
          ...
          top_data_1[(i / axis_dist * top_k_ + j) * axis_dist + i % axis_dist]
            = bottom_data_vector[j].first;
        } else {
          // Produces max_ind and max_val
          top_data_1[2 * i * top_k_ + j] = bottom_data_vector[j].second;
          top_data_1[2 * i * top_k_ + top_k_ + j] = bottom_data_vector[j].first;
        }
      } else {
        // Produces max_ind per axis
        top_data_1[(i / axis_dist * top_k_ + j) * axis_dist + i % axis_dist]
            = bottom_data_vector[j].second;
        if (bottom_data_vector[j].second==6) {
          top_data_2[(i / axis_dist * top_k_ + j) * axis_dist + i % axis_dist]
            = 255;
