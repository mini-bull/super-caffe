...
void VideoDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (video_type_ == VideoDataParameter_VideoType_WEBCAM) {
      ...
      ++processed_frames_;
      cap_ >> cv_img;
      //cv::imwrite("/home/wissen/caffe-ssd_without_fcn/test.jpg", cv_img);
      //LOG(INFO)<<" cv_img.wigth: "<<cv_img.rows<<" cv_img.height: "<<cv_img.cols<<" cv_img.channels: "<<cv_img.channels();
    } else {
