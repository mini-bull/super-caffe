...
template <typename Dtype>
__global__ void add_kernel_new(const int n, const Dtype alpha, const Dtype* a, const Dtype beta, const Dtype* b, 
																																															Dtype* y) 
{
  CUDA_KERNEL_LOOP(index, n) 
  {
...
template <>
void caffe_gpu_add_new<float>(const int N, const float alpha, const float* a, const float beta, const float* b,
    float* y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
  add_kernel_new<float><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>
  (N, alpha, a, beta, b, y);
}

template <>
void caffe_gpu_add_new<double>(const int N, const double alpha, const double* a,  const double beta, const double* b,
    double* y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
  add_kernel_new<double><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>
  (N, alpha, a, beta, b, y);
}
