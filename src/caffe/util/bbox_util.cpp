...
template <typename Dtype>
void VisualizeBBox(const vector<cv::Mat>& images, const vector<cv::Mat>& segs, const vector<cv::Mat>& imgsegs,
                   const Blob<Dtype>* detections, const float threshold, const vector<cv::Scalar>& colors,
  ...
  /////////////////////////////////////////////////
  vector<LabelBBox> all_segs(num_img);
  all_segs = Jointdis(segs);
  for (int i = 0; i < num_det; ++i) {
    ...
    if (score < threshold) {
      ...
      for (map<int, vector<NormalizedBBox> >::iterator it =
          all_segs[img_idx].begin(); it != all_segs[img_idx].end(); ++it) {
        int seg_label = it->first;
        const vector<NormalizedBBox>& seg_bboxes = it->second;
        for (int j = 0; j < seg_bboxes.size(); ++j) {
          if (label==seg_label && detections_data[i * 7 + 3] * width>seg_bboxes[j].xmin() && detections_data[i * 7 + 5] * width>seg_bboxes[j].xmin()
          && detections_data[i * 7 + 3] * width<seg_bboxes[j].xmax() && detections_data[i * 7 + 5] * width<seg_bboxes[j].xmax() 
          && detections_data[i * 7 + 4] * height>seg_bboxes[j].ymin() && detections_data[i * 7 + 6] * height>seg_bboxes[j].ymin()
          && detections_data[i * 7 + 4] * height<seg_bboxes[j].ymax() && detections_data[i * 7 + 6] * height<seg_bboxes[j].ymax()) {
            NormalizedBBox bbox;
  ...
  for (int i = 0; i < num_img; ++i) {
    ...
    cv::Mat seg = segs[i];
    cv::Mat imgseg = imgsegs[i];
    //cv::imwrite("/home/wissen/caffe-ssd_without_fcn/test.jpg", imgseg);
    // Show FPS.
    ...
    cv::Mat newImage = AddChannels(image,imgseg);
    cv::imshow("detections", newImage);
    if (cv::waitKey(1) == 27) {
      raise(SIGINT);
    }
  }
...
////////////////////////////////////////////////////////
cv::Mat AddChannels(cv::Mat image, cv::Mat seg) {
  cv::Mat imageBlue, imageRed, imageGreen, NewSeg;
  vector<cv::Mat> channels;

  cv::split(image, channels);
  imageGreen = channels.at(1);

  cv::threshold(seg, NewSeg, 200, 1, CV_THRESH_BINARY);
  NewSeg = NewSeg.mul(0.7);
  imageGreen = imageGreen.mul(NewSeg);
  cv::addWeighted(imageGreen,1.0,seg,0.3,0.0,imageGreen);
...
///////////////////////////////////////////////////////////
vector<LabelBBox> Jointdis(vector<cv::Mat> images) {
  int num_img = images.size();
  vector<LabelBBox> all_detections(num_img);
  ...
  for (int j = 0; j < num_img; j++) {
    for (int k = 0; k < 20; k++) {
      ...
      //cv::imwrite("/home/wfw/caffe-ssd/FeatureVision/img_"+s1.str()+"_"+s2.str()+".png", image);
      if (k==0) {
        cv::threshold(image, binaryM, k, 255, CV_THRESH_BINARY);
        continue;              
      }
      cv::threshold(image, binaryM, k, 255, CV_THRESH_TOZERO_INV);
      cv::threshold(binaryM, binaryM, k-1, 255, CV_THRESH_BINARY_INV);
      cv::copyMakeBorder(binaryM,binaryM,50,50,50,50,IPL_BORDER_CONSTANT,cv::Scalar(255));
      //cv::imwrite("/home/wfw/caffe-ssd/FeatureVision/binary_"+s1.str()+"_"+s2.str()+".png", binaryM);
      cv::findContours(binaryM, contours, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE);
      for (int l =0; l < contours.size(); l++){
        float contours_area = abs(cv::contourArea(contours[l]));
        if (contours_area>1000&&contours_area<500000) {
          cv::Rect rect = cv::boundingRect(contours[l]);
          rect.x = rect.x - 50;
          rect.y = rect.y - 50;
          ...
          NormalizedBBox bbox;
          bbox.set_xmin(rect.x);
          bbox.set_ymin(rect.y);
          bbox.set_xmax(rect.x+rect.width);
          bbox.set_ymax(rect.y+rect.height);

          cv::rectangle(image, rect, cv::Scalar(255,0,0), 1, 1, 0);
          // cv::imwrite("/home/wfw/caffe-ssd/FeatureVision/binary_"+s1.str()+"_"+s2.str()+".png", image);

          all_detections[j][k+1].push_back(bbox); 
        }
      }
    }
  }
...
void VisualizeBBox(const vector<cv::Mat>& images,
                   const vector<cv::Mat>& segs,
                   const vector<cv::Mat>& imgsegs,
